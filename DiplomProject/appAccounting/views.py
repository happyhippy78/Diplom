...
def index(request):
    ...
    try:
        ...
        plt.savefig('appAccounting/static/appAccounting/expense.jpg')
    except TypeError:
        print('Нет даты.')
    context = {'expense_items':expense_items,'budget':budget_total['budget'],'expenses':abs(expense_total['expenses'])}
    return render(request,'appAccounting/index.html',context=context)
...
def sign_up(request):
    if request.method == 'POST':
        ...
        return render(request,'appAccounting/sign_up.html',{'form':form})